// this is a modified version of powerglitch and react-powerglitch libraries.
// this differs only in fact that `stopGlitch` pauses animation instead of cancelling it.
...
const getDefaultOptions = (
  ...
  return {
    ...
    timing:
      playMode === "always"
        ? { duration: 2 * 1000, iterations: Infinity }
...
const generateGlitchSliceLayer = (options: PowerGlitchOptions) => {
  ...
  for (let index = 0; index < stepCount; ++index) {
    ...
    if (options.slice.hueRotate) {
      styles.filter = `hue-rotate(${Math.floor(
        getGlitchRandom(options, index / stepCount) * 360
      )}deg)`;
    }
    steps.push(styles);
  }
...
const generateBaseLayer = (options: PowerGlitchOptions): LayerDefinition => {
  ...
  for (let index = 0; index < stepCount; ++index) {
    const translateX =
      getGlitchRandom(options, index / stepCount) *
...
const generateLayers = (options: PowerGlitchOptions): LayerDefinition[] =>
  [
    ...
    ...Array.from({ length: options.slice.count }).map(() =>
      generateGlitchSliceLayer(options)
    ),
 ...
 * @returns
 */
const prepareGlitchElement = (
  element: HTMLElement,
  options: PowerGlitchOptions
): {
  glitched: HTMLElement;
  ...
  (layersContainer.firstElementChild as HTMLDivElement)
    .getAnimations()
...
const glitchElement = (
  ...
  const { glitched, container, layersContainer } = prepareGlitchElement(
    element,
    options
  );
 ...
 */
const glitch = (
  elOrSelector: GlitchableElement = ".powerglitch",
  userOptions: GlitchPartialOptions = {}
): GlitchResult => {
  // Fix options with defaults
  const options: PowerGlitchOptions = mergeOptions(
    getDefaultOptions(userOptions.playMode),
    userOptions
  );
  ...
  if (
    options.optimizeSeo &&
    /google|bing|baidu|yandex|slurp|bot/i.exec(navigator.userAgent)
  ) {
    return {
      containers: [],
      startGlitch: () => {
...
export function useGlitch(
  userOptions?: RecursivePartial<PowerGlitchOptions>
): GlitchHandle {
  /**
